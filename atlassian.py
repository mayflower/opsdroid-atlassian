...
def re_botcmd(
    pattern, prefixed=True, matchall=False, admin_only=False, split_args_with=None
):
    if split_args_with is None:
        split_args_with = r"\s"
        ...
        if not hasattr(fn, "_opsdroid_adapted"):
            ...
            async def wrapped(self, opsdroid, config, message, *args, **kwargs):
                if not message.user_id.endswith(":matrix.mayflower.de"):
                    return
                if admin_only and message.user_id not in self.config.get(
                    "admin_ids", ["linus.heckemann:matrix.mayflower.de"]
                ):
                    await message.respond("You're not allowed to do that!")
                    return
                args = re.split(
                    split_args_with, re.sub(pattern, "", message.text, count=1)
                )
...
class Atlassian(Skill):
    ...
    def get_route(self, project, room):
        ...
        return self.config["projects"].get(project, {}).get("routes", {}).get(room)

    def get_routes(self, project):
        """Fetch the routes for a project.
        Always check if the project exists before calling this.
        """
        return list(self.config["projects"].get(project, {}).get("routes", {}).keys())
    ...
    def save_config(self):
        ...
        plugin.
        """
        self._bot.plugin_manager.set_plugin_configuration("Atlassian", self.config)
    ...
    def show_project_config(self, project):
        ...
        if self.has_project(project):
            ...
            for room in self.get_routes(project):
                message.append(
                    " • {0} for events: {1}".format(
                        room, " ".join(self.get_events(project, room))
                    )
                )
            return "\n".join(message)
        else:
    ...
    def atlassian_help(self, *args):
        ...
        message.append(
            " • route <project> <room> <events>: to relay "
        ...
        message.append(" • defaults: to show the events to be forwarded " "by default")
    ...
    def atlassian_routes(self, message, args):
        ...
        if args:
            ...
            if projects:
                yield (
                    "You asked for it, here are all the projects, the "
    ...
    def atlassian_remove(self, message, args):
        ...
        if len(args) == 1:
            ...
            if not self.get_routes(project):
                self.clear_project(project)
                yield (
                    "No more routes for {0}, removing remaining "
    ...
    def receive(self, request):
        ...
        if self.get_project(project) is None and not global_event:
            ...
            # discard the message
            log.info(
                "Message received for {0} but no such project "
    ...
    def join_and_send(self, room_name, message):
        ...
        if isinstance(message, dict):
            self.send_card(to=room, **message)
        else:
    ...
    def msg_issue_generic(body, project, event_type=None):
        ...
        if "changelog" in body:
            ...
            for item in body["changelog"]["items"]:
                ...
                changes.append(
                    (field, "{} → {}".format(item["fromString"], item["toString"]))
                )
        ...
        if "comment" in body:
            url = "{base_url}/browse/{key}?focusedCommentId={commentId}&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel#comment-{commentId}".format(
                base_url=base_url, key=key, commentId=body["comment"]["id"]
            )
